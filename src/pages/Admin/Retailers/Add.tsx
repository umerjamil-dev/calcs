...
const AdminAddRetailer = () => {
  const navigate = useNavigate()
  const store = useRetailerStore()

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const success = await store.addRetailer()
    if (success) {
        ...
        <header className="sticky top-0 z-10 border-b border-slate-200 bg-white px-6 py-3">
          <div className="mx-auto flex max-w-7xl items-center justify-between">
            <div className="flex items-center gap-3">
              <a href="/admin/retailers" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100">
                <ArrowLeft size={16} />
              </a>
              <h1 className="text-lg font-semibold text-slate-800">Add Retailer</h1>
            </div>
            <Button
              type="submit"
              form="retailer-form"
            ...
            >
              <Save size={16} />
              {store.adding ? 'Saving...' : 'Save Retailer'}
            </Button>
          </div>
        </header>

        <main className="mx-auto px-6 py-6">
          <form id="retailer-form" onSubmit={handleSubmit} noValidate>
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="mb-6 text-base font-semibold text-slate-800">Retailer Information</h2>
              ...
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
                ...
                <Field label="Company / Shop Name *" id="companyName" value={store.companyName} onChange={(v) => store.setField('companyName', v)} placeholder="Ali Petroleum" error={store.errors.companyName} />
